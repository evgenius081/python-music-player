...
class SongList(AspectFrame):
    def __init__(self, media_player):
        ...
        self.__media_player.connect("source_changed", self.__current_song_updated)
    ...
    def __create_UI(self):
        ...
        self.__side_margin = int((config.WINDOW_WIDTH - config.TILE_WIDTH) / 2) - config.NUMBER_LABEL_WIDTH
        self.__box.set_margin_start(self.__side_margin)
        ...
        self.__box.set_size_request(
            config.WINDOW_WIDTH - self.__side_margin,
    ...
    def __create_header(self):
        ...
        self.__header_box.set_valign(Gtk.Align.START)
        ...
        self.__duration_icon.set_size_request(DURATION_ICON_SIZE, DURATION_ICON_SIZE)
        self.__header_box.append(self.__duration_icon)

    def __create_list(self):
        self.__scroll_area = ScrolledWindow()
        self.__scroll_area.set_size_request(
            config.WINDOW_WIDTH - self.__side_margin,
            config.WINDOW_HEIGHT - config.MENU_BAR_HEIGHT - config.CONTROL_BLOCK_HEIGHT - LAYOUT_VERTICAL_MARGIN * 2
        )
        self.__scroll_area.set_name("song_scroll")
        self.__scroll_area.set_hexpand(config.TILE_WIDTH)
        self.__scroll_area.set_halign(Gtk.Align.START)
        ...
        self.__song_box.set_hexpand(True)
    ...
    def __song_deleted(self, _, song):
        song_to_remove = \
        ...
        self.__song_box.remove(song_widget_to_remove)
        self.__song_widgets.remove(song_widget_to_remove)
        self.__current_song_index = self.__songs.index(self.__media_player.get_current_song())
        for index, song_widget in enumerate(self.__song_widgets):
            song_widget.set_song_number(index + 1)

    def __songs_added(self, _, new_songs):
